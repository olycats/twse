...
for dt in month_range(start_dt, end_dt):
    ...
    path = os.path.join(os.getcwd(), "FMTQIK/") + dt.strftime("%Y%m%d") + ".csv"
...
for dt in date_range(start_dt, end_dt):
    ...
    path = os.path.join(os.getcwd(), "MI_5MINS/") + dt.strftime("%Y%m%d") + ".csv"
    download_data(url, path)
...
for dt in date_range(start_dt, end_dt):
    ...
    path = os.path.join(os.getcwd(), "MI_5MINS_INDEX/") + dt.strftime("%Y%m%d") + ".csv"
    download_data(url, path)
